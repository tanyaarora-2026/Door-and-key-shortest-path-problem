    ...
    for i in range(env.unwrapped.height):
        for j in range(env.unwrapped.width):
            if isinstance(env.unwrapped.grid.get(j, i), Key):
                ...
                # if env.unwrapped.grid.get(j, i).is_open:
                #     info["door_open"]=(True,)
                # else:
                #     info["door_open"]=(False,)
...
def load_random_env(path):#env_folder):
    """
...
#     return
  ...
  # <- important: use v2 API

def draw_gif_from_seq(seq, env, path="./gif/doorkey.gif", frame_duration=8.0):
    with imageio.get_writer(path, mode="I", duration=frame_duration, loop=0) as w:
        img = env.render()
        w.append_data(img)
        for act in seq:
            step(env, act)
...
def to_int(x: Any) -> int:
    """Return a plain Python int from numpy/Python scalar-like values."""
    return int(x)

def to_bool(x: Any) -> bool:
    """Return a plain Python bool from numpy/Python bool-like values."""
    return bool(x)

def to_tuple(xy: Union[Iterable[int], np.ndarray, tuple, list]) -> Tuple[int, int]:
    a = np.asarray(xy).reshape(2)
    return (int(a[0]), int(a[1]))

def to_cell_tuple_list(arrs: Iterable[Union[np.ndarray, Iterable[int]]]) -> List[Tuple[int, int]]:
    return [to_tuple(a) for a in arrs]

def to_cell_tuple_set(arrs: Iterable[Union[np.ndarray, Iterable[int]]]) -> set[Tuple[int, int]]:
    return set(to_cell_tuple_list(arrs))
